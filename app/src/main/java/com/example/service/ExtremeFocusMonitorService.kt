...
import com.example.domain.ToughLoveQuoteEngine
...
class ExtremeFocusMonitorService : Service() {
    ...
    private fun startUsageMonitoringLoop() {
        ...
        monitorJob = serviceScope.launch {
            ...
            while (isActive) {
                try {
                    checkAndSyncUsageThresholds()
                } catch (e: Exception) {
                    Log.e(TAG, "Error in usage monitor loop: ${e.message}", e)
                }
                // Check usage every 3 seconds for accurate, prompt detection without battery drain
                delay(3000L)
            }
        }
    }

    private suspend fun checkAndSyncUsageThresholds() {
    ...
    companion object {
        ...
        const val ACTION_START_SERVICE = "com.example.service.action.START_MONITOR"
        const val ACTION_STOP_SERVICE = "com.example.service.action.STOP_MONITOR"
